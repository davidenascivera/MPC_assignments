    ...
    def run(self, x0):
        ...
        for i in range(sim_loop_length):
            ...
            if self.estimation_in_the_loop is False:
                ...
                # Get measurement
                '''
                    Part that add uncertainty RAND
                '''
                ...
                trace_P = np.trace(self.model.kf_estimator.P)
                print(trace_P)


                #print(f"Trace of covariance matrix P at time step {i}: {trace_P}\n")
                #self.trace_P_list.append(trace_P)
                #print(self.trace_P_list)
