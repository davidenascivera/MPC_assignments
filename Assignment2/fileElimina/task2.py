...
sim_env.visualize_prediction_vs_reference(
    x_pred=x_star, x_ref=x_full_ref, control=u_star
)
sim_env.visualize_state_vs_reference(state=y, ref=x_full_ref, control=u_star)

""" 
Q5: Broken truster simulation
"""
...
R = np.eye(4) * 10
ctl_wz = FiniteOptimization(
    queen,
    queen.linearized_discrete_dynamics,
    total_time=40.0,
    rendezvous_time=35.0,
    ref_type="3d",
    R=R,
)
...
# Simulate 3D
sim_env = EmbeddedSimEnvironment(
    model=queen, dynamics=queen.linearized_discrete_dynamics, controller=u, time=40.0
)
t, y, u = sim_env.run(x0, x_ref=x_full_ref)
sim_env.visualize_error()
