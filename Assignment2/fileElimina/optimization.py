...
class FiniteOptimization(object):
    ...
    def __init__(
        ...
        xr: dict[int : cp.Parameter] = {
            step: cp.Parameter((self.Nr, 1))
            for step in range(self.Ntr, self.Ntr + (self.Nt - self.Ntr))
        }

        self.parameters = {
            "reference_trajectory": xr,
            "initial_state": x0,
            "initial_input": u0,
        }
        ...
        self.x_vars_dict = {
            step: cp.Variable((self.Nx, 1)) for step in range(self.Nt + 1)
        }
        ...
        for t in range(self.Nt):
            ...
            con_eq.append(
                (x_t_next - self.x_vars_dict[t + 1]) == 0
            )  # 2) Dynamics constraint
            ...
            if t > self.Ntr:
                ...
                if ref_type == "2d":
                    ...
                    # and tolerances as
                    #
                    #   self.pos_tol = 0.001 * np.ones((2, 1))  # position in X, Y
                    #   self.att_tol = 0.001                    # angle in radians
