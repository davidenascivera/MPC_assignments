"""
Model Predictive Control - CVXPY interface
...
class FiniteOptimization(object):
    ...
    def __init__(self, model, 
        ...
        xr : dict[int:cp.Parameter] = {step:cp.Parameter((self.Nr,1)) for step in range( self.Ntr,  self.Nt)}
        ...
        for t in range(self.Nt):
            ...
            con_eq.append((x_t_next - self.x_vars_dict[t + 1]) == 0) 
            ...
            if t > self.Ntr:
                ...
                if ref_type == "2d":
                    ...
                    con_ineq.append(x_ref[0:1]-x_t[0:1] <= self.pos_tol) 
                    con_ineq.append(x_ref[0:1]-x_t[0:1] >= -self.pos_tol)
                    
                    con_ineq.append(x_ref[2]-x_t[4] <=  self.pos_tol) 
                    con_ineq.append(x_ref[2]-x_t[4] >= -self.pos_tol) 
    ...
    def solve_problem(self, x0, xr):
        ...
        # Solve NLP
        sol    = self.solver.solve(verbose = True,solver=cp.MOSEK)
        solve_time += time.time()
